...
（8）sudo apt-get install python-rosinstall python-rosinstall-generator python-wstool buid-essential
     sudo apt-get update
     sudo apt-get install

（9）测试（roscore节点管理器）。一旦启动roscore后，便可以运行ROS程序了。

（10）如果需要的话，可通过sudo gedit ~/.bashrc修改.bashrc中的内容。
